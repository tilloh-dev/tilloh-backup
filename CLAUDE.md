...
- **amdgpu runtime-suspends the card 5 s after the last request and evicts the model into host RAM.** Root cause found 2026-09-24 after three wrong diagnoses. Proof, same config: a request every 10 s held 19809 MiB VRAM steady over 90 s; 15 s of idle dropped it to **26 MiB VRAM / 15944 MiB GTT**. The server keeps answering, far slower, while pinning ~16 GiB of system RAM — one episode ran two days undetected and left the box at 273 MiB free RAM with a full swap. `power/runtime_suspended_time` stood at ~72 h of a 72 h uptime. **Fix: `power/control=on` on the AMD card.** A udev rule keyed on vendor/device was written but **measured not to take effect** (the value is set before amdgpu binds and the driver overrides it) — delete it. **`amdgpu.runpm=0` in `GRUB_CMDLINE_LINUX_DEFAULT` is the working fix**, verified across a reboot on 2026-09-24: `power/control=on`, `runtime_status=active` and 60 s of idle with no eviction, without any manual step. Address the card by PCI id (`/sys/bus/pci/devices/0000:03:00.0`), not by `cardN` — installing a second GPU renumbered the AMD card from card0 to card1 on 2026-09-24.
- **Adding the RTX 3060 (2026-09-24) left the Vulkan order intact, but that is luck, not a guarantee.** Debian's `nvidia-driver` 535.309.01 from `non-free` builds against kernel 6.12.95 via DKMS (the backports archive carries no nvidia-driver at all — `-t bookworm-backports` makes apt fail on an unsatisfiable mix). It installs `nvidia_icd.json`, so llama.cpp now enumerates **`Vulkan0` = 7900 XTX, `Vulkan1` = RTX 3060** and `device = VULKAN0` still resolves to the AMD card — **re-check `--list-devices` after every driver or BIOS change**, because llama.cpp selects by index and has no name-based option. The box is headless (no connector on either card is `connected`), so there is no Xorg or display-provider risk. Rollback if a driver update breaks the build: `sudo apt purge '~nnvidia'` — `amdgpu` is in-kernel and unaffected.
- **The same preset needed 23419 MiB before that reboot and 24186 MiB after it** — unexplained; both measurements are post-runtime-PM-fix, so it is not the old eviction. Candidates are the nvidia driver's presence or `runpm=0`, not separated (that would mean removing the driver again). The daily driver therefore runs at ctx **229376**, not the full native 262144: that keeps 1332 MiB free instead of 374, and the throughput difference is not resolvable at n=4 per point (median 55.0 vs 52.7 t/s, ranges 48.9–60.0 vs 50.2–62.1).
- **The "hard 22.5 GB VRAM cliff" in the old preset header does not exist** — it was this eviction. The header claimed throughput collapses by 3x above 22461 MiB and that 22854 MiB still gave 22 t/s. Measured after the fix: **23002 MiB occupied -> 60.6 t/s median**, the fastest figure on that box, and 200 s of idle without any eviction. Every throughput number taken on Gertrude before 2026-09-24 14:00 was recorded under the eviction regime and is suspect.
- `llama.cpp/tools/healthcheck.sh` stays as the safety net: systemd timer, 60 s, restarts the unit after 2 consecutive hits, logs every one to `~/.local/state/llama.cpp/healthcheck.log`. It finds the AMD card by probing for `mem_info_vram_used` rather than a fixed `cardN`, which is why it survived the renumbering. It caught a real incident on 2026-09-24 and freed 24 GB of RAM. `LLAMA_HEALTHCHECK_SETTLE` (90 s) skips the check right after a unit start — a loading 16 GiB model looks exactly like an eviction from outside (measured mid-transition samples vram=9020/gtt=15877 and vram=801/gtt=16719), and acting on one drives a self-sustaining restart loop. Auto-update stays fully on; only the check's own restart skips it, and the throttle marker is restored so the regular schedule is not delayed. **The VRAM/GTT signal does not transfer to hermine**: there the Windows driver spills into shared memory while `nvidia-smi` "used" still looks fine, so that host needs a throughput probe (`detect_*` is pluggable for it).
