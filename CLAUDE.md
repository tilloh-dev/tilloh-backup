...
- **The GPU can evict a loaded model into host RAM at runtime, silently.** Measured on Gertrude 2026-09-24: one `llama-server` sat at 20405 MiB VRAM / 1610 MiB GTT right after loading and at **26 MiB VRAM / 15986 MiB GTT** minutes later — same process, no restart. It keeps answering, far slower, while pinning ~16 GiB of system RAM; an earlier episode ran undetected for two days and left the box at 273 MiB free RAM with a full swap. `fit = off` lets the fallback through without a warning. **A one-shot check after loading cannot see this** — three separate load-time reproductions all looked clean, which is why the fault was misdiagnosed twice. `llama.cpp/tools/healthcheck.sh` polls for it (systemd timer, 60 s; restarts the unit after 2 consecutive hits and logs every one to `~/.local/state/llama.cpp/healthcheck.log`). Its trigger is unknown — the log exists to collect the circumstances. Auto-update stays fully on; only the health check's own restart skips the update (a heavier build could stop fitting and put the check in a restart loop, and `bootstrap.sh` `rm -rf`s the old one), and it puts the throttle marker back afterwards so the regular schedule is not delayed. **The amdgpu VRAM/GTT signal does not transfer to hermine**: there the Windows driver spills into shared memory while `nvidia-smi` "used" still looks fine, so that host needs a throughput probe (`detect_*` in the script is pluggable for it).
