...
Vision-capable models need a separate **multimodal projector**
(`mmproj-*.gguf`) alongside the LM GGUF, referenced via the preset key
`mmproj = …`. `gemma-4-31B` and both Qwen3.6 models are VLMs; no `mmproj` is
currently downloaded for them, so the committed presets run text-only.
