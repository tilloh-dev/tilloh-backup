...
**Renamed 2026-08-15 (same day):** the twins are now `[Qwen3.8-27B-small]` (ex `[Qwen3.8-27B]`) and `[Qwen3.8-27B-large]` (ex `[Qwen3.8-27B-maxctx]`) in both preset files and in the hermine OpenCode provider (models map + whitelist, live and backup). The local Vulkan provider keeps the plain `Qwen3.8-27B` ID, so the rename stayed hermine-side; renaming the OpenCode entry without touching the local machine's models.ini would have broken the ID linkage. This doc file keeps the base-model name.
...
## RPC quality preset `[Qwen3.8-27B-UD-Q8_K_XL-260ctx-q8_0-rpc]` — removed 2026-09-22

The section and its whole measurement record are gone. It ran UD-Q8_K_XL (29.3 GiB) at full
native ctx 262144 by pooling hermine's 4090 with the 7900 XTX over RPC, and every number in it
— the fit-params ladder, `tensor-split = 38,27`, the sidecar-drafter gate (`device-draft`,
`spec-draft-n-max = 6`, `spec-draft-p-min = 0.5`), the throughput figures — was measured with
that card in `lieselotte` (i9-14900K). The XTX now sits in `Gertrude` (i9-9900K, 31 GB RAM) and
lieselotte has no GPU any more, so the pool no longer exists in the form that was measured and
the numbers are not transferable. The preset was already absent from the live `models.ini`.
Re-measure from scratch if a Gertrude/hermine pool is ever built.
